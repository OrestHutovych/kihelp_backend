...
public class TaskProcessUseCaseFacade implements TaskProcessUseCase {
    ...
    public Map<String, String> processTask(TaskProcessCreateDto request) {
        ...
        if (!hasRole(targetUser, "ROLE_ADMIN") && !isTaskDeveloper(targetUser, task)) {
            ...
            price = calculateDiscountPrice(task.getPrice(), discount);
        ...
        Map<String, String> processResponse = new HashMap<>(Map.of(
                "taskTitle", task.getTitle(),
                "subjectName", task.getTeacher().getSubject().getName(),
                "developerName", task.getDeveloper().getUsername()
        ));

        if(task.isAutoGenerate()){
            processAutoGeneratedTask(targetUser, task, request, processResponse, price);
        }else{
            processManualTask(targetUser, task, request, processResponse, price);
        }
    ...
    private void processAutoGeneratedTask(User targetUser,
                                          Task task,
                                          TaskProcessCreateDto request,
                                          Map<String, String> processResponse,
                                          BigDecimal price) {
    ...
    private void processManualTask(User targetUser,
                                   Task task,
                                   TaskProcessCreateDto request,
                                   Map<String, String> processResponse,
                                   BigDecimal price) {
